...
public class TokenService {
  ...
  public TokenService(@Value("${auth.token}") String token, @Value("${token.bearer}") String bearer) {
    this.token = token;
    this.bearer = bearer;
  }
  ...
  public boolean verifyToken(String authHeader) {
    String tokenFromHeader = authHeader.replace(bearer, Strings.EMPTY);
    return tokenFromHeader.equals(token);
  }
